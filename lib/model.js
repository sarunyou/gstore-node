...
class Model extends Entity {
    ...
    static deleteAll(ancestors, namespace, cb) {
        ...
        let entities;
        let totalBatches;
        const limitQuery = 100000;

        return createQueryLimit(limitQuery).run().then(onEntities, onError);

        // ------------------------------------------------

        function createQueryLimit(limitQuery) {
            // We query only limit number incase of big table
